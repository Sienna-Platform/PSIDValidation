...
function add_inv_case78!(sys, static_device)
...
function add_inv_darco_droop!(sys, static_device)
...
function add_inv_gfoll!(sys, static_device)
    dyn_device = PSY.DynamicInverter(
        get_name(static_device),
        1.0, #ω_ref
        converter_low_power(), #converter
...
function replace_with_source!(sys, static_device)
    ...
    bus = get_bus(static_device)
    remove_component!(sys, static_device)
