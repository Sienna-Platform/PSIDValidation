...
pscad_workspace_name = "workspace_3bus.pswx"
pscad_case_name = "case_3bus"
...
quantities_to_record = [ (:V, "Bus_2")]  #Note: Add more signals to save here
setup_output_channnels(project, quantities_to_record, (15, 2))

############# SETUP INITIALIZATION CHANNELS ###################################
PP.update_parameter_by_name(project.find("master:const", "t_INV"), "Value", 1.0)
...
for signal_name in filter!(x-> x !== "time",names(df))
    display(plot(df[3:end, "time"], df[3:end, signal_name], label = signal_name, xlim = (0.0, 2.5), ylim=(0.0,2.03)))
end
