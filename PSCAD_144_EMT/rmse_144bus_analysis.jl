...
for c in get_components(DynamicGenerator, sys)
    machine = get_machine(c)
    set_R!(machine, get_Xd_p(machine)*0.01)
end
...
eig = small_signal_analysis(sim)
sei = summary_eigenvalues(eig)
sei[sei[!, "Freq [Hz]"] .<= 65, :][10:35, :]
